-- Uber Eats delivery link (alongside foodpanda_url), owner-editable (low-risk).
alter table public.locations add column if not exists ubereats_url text;
...
declare
  ...
  v_cur jsonb;
  low_fields  text[] := array['phone','instagram','line_id','foodpanda_url','ubereats_url','description',
