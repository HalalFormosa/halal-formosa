-- More social/contact links: facebook, tiktok, website (owner-editable, low-risk).
...
declare
  ...
  v_cur jsonb;
  low_fields  text[] := array['phone','instagram','facebook','tiktok','website','line_id',
                              'foodpanda_url','ubereats_url','description',
...
begin
  ...
  if v_low <> '{}'::jsonb then
    update public.locations set
      ...
      facebook              = case when v_low ? 'facebook'              then v_low->>'facebook'              else facebook end,
      tiktok                = case when v_low ? 'tiktok'                then v_low->>'tiktok'                else tiktok end,
      website               = case when v_low ? 'website'               then v_low->>'website'               else website end,
      ...
      ubereats_url          = case when v_low ? 'ubereats_url'          then v_low->>'ubereats_url'          else ubereats_url end,
