...
function updateLocale(filePath, adminKeys, merchantKeys) {
    ...
    data.admin = { ...data.admin, ...adminKeys };
    
    // Update merchant section
    if (!data.merchant) data.merchant = {};
    if (!data.merchant.register) {
        data.merchant.register = merchantKeys.register;
    } else {
        // Merge hints
        data.merchant.register.hints = { 
            ...data.merchant.register.hints, 
            ...merchantKeys.register.hints 
        };
        // Merge other register keys
        Object.keys(merchantKeys.register).forEach(key => {
            if (key !== 'hints') {
                data.merchant.register[key] = merchantKeys.register[key];
            }
        });
    }
...
const enMerchant = {
    register: {
      ...
      rejectedDesc: "Unfortunately, your merchant application was not approved at this time.",
      rejectionReason: "Reason for Rejection",
      rejectedBanner: "Update Your Application",
      rejectedBannerDesc: "Your previous application was not approved. Please review the reason below and update your information before resubmitting.",
      rejectedProfileDesc: "Your application was not approved. Click below to view feedback and revise."
    }
};
...
const zhMerchant = {
    register: {
      ...
      rejectedStatus: "申請未通過",
      rejectedDesc: "很抱歉，您的商家申請目前未能通過核准。",
      rejectionReason: "未通過原因",
      rejectedBanner: "更新您的申請資訊",
      rejectedBannerDesc: "您之前的申請未獲核准。請參考下方的未通過原因，並在重新提交前更新您的資訊。",
      rejectedProfileDesc: "您的申請未獲核准。請點擊下方查看原因並進行修正。"
    }
};

updateLocale('c:/Users/User/halal-project/src/locales/en.json', enAdmin, enMerchant);
updateLocale('c:/Users/User/halal-project/src/locales/zh.json', zhAdmin, zhMerchant);

// Add common.reapply
function addCommonKey(filePath, key, value) {
  const data = JSON.parse(fs.readFileSync(filePath, 'utf8'));
...
addCommonKey('c:/Users/User/halal-project/src/locales/en.json', 'reapply', 'Re-apply');
addCommonKey('c:/Users/User/halal-project/src/locales/zh.json', 'reapply', '重新申請');
