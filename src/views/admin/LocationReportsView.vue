<template>
  <ion-page>
    <ion-header>
      <app-header
          :title="$t('profile.admin.locationReports')"
          :icon="listOutline"
          :showBack="true"
          backRoute="/profile"
      />
    ...
    <ion-content class="ion-padding">
      ...
      <ion-list v-else-if="reports.length">
        <ion-item v-for="report in reports" :key="report.id" button @click="openReport(report)">
          <ion-label>
            <h2>{{ report.locations?.name || 'Unknown Location' }}</h2>
      ...
      <ion-modal :is-open="!!selectedReport" @didDismiss="selectedReport = null">
        ...
        <ion-content class="ion-padding" v-if="selectedReport">
          <ion-list>
            <ion-item>
              <ion-label>
                <h2>{{ $t('admin.locationName') || 'Location' }}</h2>
                <p>{{ selectedReport.locations?.name }}</p>
              </ion-label>
            </ion-item>
            <ion-item>
              <ion-label>
...
async function updateStatus() {
  ...
  if (!error) {
    fetchReports()
    // Update local list too if we want immediate feedback without full reload
